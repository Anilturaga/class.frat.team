...
class Game{
	...
	constructor(){
		...
		options.assets.push(`${game.assetsPath}fbx/noground.fbx`);
	...
	loadEnvironment(loader){
		...
		loader.load(`${this.assetsPath}fbx/noground.fbx`, function(object){
	...
	updateRemotePlayers(dt){
		...
		//console.log("remoteplayersupdate"+" "+remotePlayers);
...
{ 
	console.log("entered in to setinterval");
	var textureCube;
	const tloader = new THREE.CubeTextureLoader();
	tloader.setPath( `assets/images/` );
	if(gate == true)
	{
		console.log("entered in to setinte11rval");
		textureCube = tloader.load( [
	...
	{
		console.log("entered in to set22interval");
		textureCube = tloader.load( [
			'px.jpg', 'nx.jpg',
			'py.jpg', 'ny.jpg',
			'pz.jpg', 'nz.jpg'
	...
	console.log("entered in to setin33terval");
	game.scene.background = textureCube;
}, 120000);
...
class Player{
	constructor(game, options){
		...
		if (options===undefined){
			const people = ['BusinessMan', 'Doctor', 'FireFighter', 'Housewife', 'Policeman', 'Roadworker', 'Robber', 'Sheriff', 'Streetman', 'Waitress'];
		...
		const loader = new THREE.FBXLoader();
		const player = this;
		//console.log("this is the player local status"+" "+player.local);


		//console.log(`${game.assetsPath}fbx/people/${model}.fbx`);
		//console.log(`${game.assetsPath}images/SimplePeople_${model}_${colour}.png`);
		loader.load( `${game.assetsPath}fbx/people/${model}.fbx`, function ( object ) {
			...
			//console.log(player);
			player.object.position.set(2500, 0, -173);
		...
		try{
			...
			if (this.game.remoteData.length>0){
				...
				for(let data of this.game.remoteData){
					//console.log("this is from the update"+this.game.remoteData);
...
class PlayerLocal extends Player{
	constructor(game, model){
		...
		const player = this;
		const socket = io('/');
		//var person = prompt("Please enter your name", "room");
		socket.emit('new room',localStorage.roomID);
		socket.on('setId', function(data){
			console.log("the id is transmitting in a room"+data.id);
			player.id = data.id;
		});
	...
	move(dt){
		...
		if (colliders!==undefined){ 
			...
			if (intersect.length>0){
				if (intersect[0].distance<150) blocked = true;
			}
		}
		...
		if (colliders!==undefined){
			...
			if (intersect.length>0){
				if (intersect[0].distance<150) this.object.translateX(100-intersect[0].distance);
			}
			...
			if (intersect.length>0){
				if (intersect[0].distance<150) this.object.translateX(intersect[0].distance-100);
			}
